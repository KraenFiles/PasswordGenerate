...
void Generate::on_generateButton_clicked()
{
	...
	else
	{
		...
		{
			GeneratePassword* gen = new GeneratePassword(GeneratePassword::ExceptionSpecialSymbols, ui->lineEditSimbols->text().toStdString());
			ui->lineEditPassword->setText(QString::fromStdString(gen->generate()));
		}
	}
}
...
void Generate::on_saveButton_clicked()
{
	...
	if(!tag.isEmpty())
	{
		...
		if(!login.isEmpty())
		{
			...
			if(!passwords.isEmpty())
			{
				...
				foreach(const QJsonValue& value, passwords)
				{
					...
					if(obj["tag"].toInt() == tagIndex && obj["login"].toInt() == loginIndex){
						...
						if(QMessageBox::question(this, "Пароль существует", "Данный пароль уже существует хотите изменить?"))
						{
			...
			{
				QJsonObject newPassword = QJsonObject({
													 qMakePair(QString("tag"), tagIndex),
